...
N_decays = 10000 # number of generated decays
...
def decay_generator():
    ...
    lab_p_cpi = fl.boost(
        np.asarray([np.full(N_decays, cl.E_cpi_tot), kaon_p_cpi[:, 0], kaon_p_cpi[:, 1], kaon_p_npi[:, 2]]).T)[:, 1:]
    lab_p_npi = fl.boost(
        np.asarray([np.full(N_decays, cl.E_npi_tot), kaon_p_npi[:, 0], kaon_p_npi[:, 1], kaon_p_npi[:, 2]]).T)[:, 1:]
    ...
    return r_decays, v_cpi_nondiv, v_npi_nondiv

misses_average = np.zeros(N_positions)
for i in range(N_iterations):
    if i/(N_iterations/10) - int(i/(N_iterations/10)) == 0:
        print("     iteration:", i)
    misses_average = (i * misses_average + fl.number_of_misses(z_detector, decay_generator())) / (i + 1)
